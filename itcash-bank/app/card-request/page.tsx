...
export default function CardRequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/cards/request", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ email, accountNumber }), // Send required data
      });
  
      const data = await response.json();
  
      if (!response.ok) throw new Error(data.msg || "Failed to request card");
  
      setIsSubmitted(true); // Successfully requested a card
    } catch (err: any) {
